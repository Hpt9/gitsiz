...
export const MapSVG = ({ 
  ...
  setRegionData, 
  allData,
  economicalZonesData // Receive the data as prop
}) => {
  ...
  const handleRegionClick = (regionName) => (e) => {
    e.preventDefault();
    if (!isRegionActive(regionName)) return;

    const regionData = mapData.filter(
      (item) => item.economical_zone[0].slug === regionName
    );

    if (regionData.length > 0) {
      setElement(regionName);
  ...
  const handleClustersChange = (selectedClusters) => {
    setFilteredClusters(selectedClusters);
  };

  const handleMethodsChange = (selectedMethods) => {
    setFilteredMethods(selectedMethods);
  };

  const getRegionOpacity = (regionSlug) => {
    if (
      filteredRegions.length === 0 &&
      filteredClusters.length === 0 &&
      filteredMethods.length === 0
    ...
    return isRegionActive(regionSlug) ? 1 : 0.3;
  };
  const getRegionName = (slug) => {
    const regionNames = {
      'qarabagh': language === 'az' ? 'Qarabağ' : language === 'en' ? 'Karabakh' : 'Карабах',
      'ganja-dashkasan': language === 'az' ? 'Gəncə-Daşkəsən' : language === 'en' ? 'Ganja-Dashkasan' : 'Гянджа-Дашкесан',
      'qazax-tovuz': language === 'az' ? 'Qazax-Tovuz' : language === 'en' ? 'Gazakh-Tovuz' : 'Газах-Товуз',
      'quba-xacmaz': language === 'az' ? 'Quba-Xaçmaz' : language === 'en' ? 'Guba-Khachmaz' : 'Губа-Хачмаз',
      'zaqatala-sheki': language === 'az' ? 'Zaqatala-Şəki' : language === 'en' ? 'Zagatala-Sheki' : 'Загатала-Шеки',
      'dagliq-shirvan': language === 'az' ? 'Dağlıq Şirvan' : language === 'en' ? 'Mountainous Shirvan' : 'Горный Ширван',
      'merkezi-aran': language === 'az' ? 'Mərkəzi Aran' : language === 'en' ? 'Central Aran' : 'Центральный Аран',
      'baku': language === 'az' ? 'Bakı' : language === 'en' ? 'Baku' : 'Баку',
      'shirvan-salyan': language === 'az' ? 'Şirvan-Salyan' : language === 'en' ? 'Shirvan-Salyan' : 'Ширван-Сальян',
      'mil-mughan': language === 'az' ? 'Mil-Muğan' : language === 'en' ? 'Mil-Mughan' : 'Миль-Мугань',
      'naxchivan': language === 'az' ? 'Naxçıvan' : language === 'en' ? 'Nakhchivan' : 'Нахчыван',
      'sherqi-zengezur': language === 'az' ? 'Şərqi Zəngəzur' : language === 'en' ? 'East Zangezur' : 'Восточный Зангезур',
      'lankaran-astara': language === 'az' ? 'Lənkəran-Astara' : language === 'en' ? 'Lankaran-Astara' : 'Ленкорань-Астара',
      'absheron-xizi': language === 'az' ? 'Abşeron-Xızı' : language === 'en' ? 'Absheron-Khizi' : 'Абшерон-Хызы'
    };
    return regionNames[slug] || slug;
  };

  const getRegionDetails = async (regionSlug) => {
    console.log(regionSlug)
    try {
      const response = await axios.get(`https://kobklaster.tw1.ru/api/economical-zones/${regionSlug}`);
      const data = response.data;
      return {
        name: data.name[language],
        population: data.population[language],
        density: data.density[language],
        zones: data.zones?.map(zone => zone.name[language]).join(', ')
      };
    } catch (error) {
  ...
  const handleMouseMove = (e, regionSlug) => {
    ...
    setTooltipPosition({ x: e.pageX+10, y: e.pageY-450 });

    // Use the data from props instead of state
    const regionData = economicalZonesData?.find(zone => zone.slug === regionSlug);
    
    if (!regionData) return;

    const regionName = regionData.name[language] || regionData.name.az;
    const population = regionData.population[language] || regionData.population.az;
    const density = regionData.density[language] || regionData.density.az;
    
    // Get zones names in current language
    const zonesNames = regionData.zones
      .map(zone => zone.name[language] || zone.name.az)
      .join(', ');

    setTooltipContent(`
      <div class="flex flex-col gap-2">
        <h3 class="text-lg font-semibold">${regionName}</h3>
        <p class="text-sm">
          ${language === 'az' ? 'Əhali' : language === 'en' ? 'Population' : 'Население'}: ${population}
        </p>
        <p class="text-sm">
          ${language === 'az' ? 'Sıxlıq' : language === 'en' ? 'Density' : 'Плотность'}: ${density}
        </p>
        ${regionData.zones.length > 0 ? `
        <p class="text-sm">
          ${language === 'az' ? 'Rayonlar' : language === 'en' ? 'Regions' : 'Районы'}: ${zonesNames}
        </p>
        ` : ''}
      </div>`
    );
  ...
  return (
    <div className="relative map_container">
      ...
      {isTooltipVisible && (
        <div 
          className="fixed bg-[#2A534F] text-white px-4 py-3 rounded-md shadow-lg z-[9999] pointer-events-none max-w-[300px]"
          style={{
...
MapSVG.propTypes = {
  ...
  economicalZonesData: PropTypes.array.isRequired,
};
